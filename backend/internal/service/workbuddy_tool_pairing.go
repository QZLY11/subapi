// Package service — WorkBuddy 出站请求体的孤儿 tool_call↔tool 配对清理。
//
// 移植自 workbuddy2api internal/upstream/tool_pairing.go。
//
// 背景：OpenAI 兼容协议要求带 tool_calls 的 assistant 消息，其每一个 tool_call id
// 都必须有对应的一条 role:tool 结果消息；反之 role:tool 消息也必须有对应的前置
// tool_call。缺任一侧，腾讯上游都会以 HTTP 400 / code=11148（tool calls and tool
// results do not match）拒绝整个请求。
//
// 工具执行失败时（参数非法、超时、工具不存在……）客户端会把 assistant 的 tool_calls
// 持久化进会话历史，却写不回结果消息。这条坏历史随后被每次请求原样重放——上游对之后
// 每一条用户消息都返回 400，整条会话报废（生产表现为「会话突然再也回不了话」）。
// 网关是最后一道防线：发出请求前剔除无法配对的条目让会话自愈，宁可丢一轮工具上下文，
// 也好过整条会话死亡。
package service
...
// cleanupWorkBuddyOrphanToolCalls 剔除无法配对的 tool_call 与 tool 结果。
//
// 语义：
...
func cleanupWorkBuddyOrphanToolCalls(messages []any) ([]any, bool) {
	...
	//
	// 注意不能用「批内每个 id 都齐才整批保留，否则删掉整个 key」：那会留下无主结果——
	// 批 [c1,c2] 只回了 c1 时，调用侧整批被删，而 tool{c1} 仍按 id 命中 keepCalls 得以
	// 保留，出站载荷变成「无 tool_calls 的 assistant + 孤儿 tool」，上游判 11148
	// （tool calls and tool results do not match）并顶死整条会话。
	// 两侧必须共用同一份 keepCalls 按 id 对称裁剪。
	for _, m := range messages {
		msg, ok := m.(map[string]any)
		if !ok {
			continue
...
// sanitizeWorkBuddyToolPairing 对已解析的出站请求体执行工具配对自愈：
// 先清理孤儿配对，再重排同批 tool 结果（顺序敏感：清理后配对关系才是权威的，
// 重排依赖清理后的结果集）。
//
// 返回是否发生改动（调用方据此决定是否需要重新序列化 body）。
func sanitizeWorkBuddyToolPairing(obj map[string]any) bool {
	messages, ok := obj["messages"].([]any)
	if !ok || len(messages) == 0 {
		return false
	}
	cleaned, changedCleanup := cleanupWorkBuddyOrphanToolCalls(messages)
	if changedCleanup {
		obj["messages"] = cleaned
	}
	repacked, changedRepack := repackWorkBuddyToolResultBlocks(cleaned)
	if changedRepack {
		obj["messages"] = repacked
	}
	return changedCleanup || changedRepack
}
